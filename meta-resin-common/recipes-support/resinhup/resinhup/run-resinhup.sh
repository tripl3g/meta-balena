...
Options:
  ...
  --supervisor-release-update
        Update the supervisor, if the version run by the device is older than the
        one shipping with the target host OS by default.
    ...
    if [ -z "$DEFAULT_SUPERVISOR_VERSION" ] || [ -z "${DEFAULT_SUPERVISOR_VERSION##*xml*}" ]; then
        ...
        if [ -z "$CURRENT_SUPERVISOR_VERSION" ]; then
            ...
            if version_gt "$DEFAULT_SUPERVISOR_VERSION" "$CURRENT_SUPERVISOR_VERSION" ; then
                log "Supervisor update: retagging from v${CURRENT_SUPERVISOR_VERSION} to v${DEFAULT_SUPERVISOR_VERSION}"
...
# Just pull the container, if requested, through cache, and do the update steps
...
function pullSupervisor() {
    ...
    log "Updating ${SUPERVISORCONF}"
    if grep -q "SUPERVISOR_IMAGE" "${SUPERVISORCONF}"; then
        sed --in-place "s|SUPERVISOR_IMAGE=.*|SUPERVISOR_IMAGE=$UPDATER_SUPERVISOR_IMAGE|" "${SUPERVISORCONF}"
    else
        echo "SUPERVISOR_IMAGE=$UPDATER_SUPERVISOR_IMAGE" >> ${SUPERVISORCONF}
    fi
    if grep -q "SUPERVISOR_TAG" "${SUPERVISORCONF}"; then
        sed --in-place "s|SUPERVISOR_TAG=.*|SUPERVISOR_TAG=$UPDATER_SUPERVISOR_TAG|" "${SUPERVISORCONF}"
    else
        echo "SUPERVISOR_TAG=$UPDATER_SUPERVISOR_TAG" >> ${SUPERVISORCONF}
    fi
...
if [ $RESINHUP_EXIT -eq 0 ] || [ $RESINHUP_EXIT -eq 2 ] || [ $RESINHUP_EXIT -eq 3 ]; then
    ...
    if [ $RESINHUP_EXIT -eq 0 ]; then
        ...
        # If this tag is set and we get to this point, then wehad a supervisor done
        if [ ! -z "$UPDATER_SUPERVISOR_TAG" ]; then
            updateSupervisor
        fi
        runPostHacks
