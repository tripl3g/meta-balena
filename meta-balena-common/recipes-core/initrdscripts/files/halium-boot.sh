...
mountroot() {
	...
	partlist="resin-data userdata UDA DATAFS USERDATA"
	...
	# 1. Mount data partition - this should probably be left to resin-mount
	...
	# BalenaOS does not provide data partition redundancy
	# On systems with A/B partition layout, current slot is provided via cmdline parameter.
	#ab_slot_suffix=$(grep -o 'androidboot\.slot_suffix=..' /proc/cmdline |  cut -d "=" -f2)
	...
	path=$(readlink -f "/dev/disk/by-state/active")
	...
	# 2. Mount the BalenaOS rootfs - we already do this so this could possible go
	...
	# 3. Mount the android image - this should be a different module
